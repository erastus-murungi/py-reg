...
class Matcher:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            def explore_node(cursor: Cursor, cur_node: State, symbol: Symbol):
                nodes = self.nfa.make_transition(cur_node, symbol)
                for node in nodes:
            ...
            def match(cursor: Cursor, cur_node: State):
                if cur_node.accepts:
                    yield cursor
                else:
                    if cursor.end < len(self.text):
                        yield from explore_node(cursor, cur_node, self.text[cursor.end])
                yield from explore_node(cursor, cur_node, EPSILON)
